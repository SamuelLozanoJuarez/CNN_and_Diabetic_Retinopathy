...
Para el entrenamiento se usarán las imágenes de los distintos repositorios y un conjunto de datos de validación y se empleará la estrategia de Early Stopping, para evitar el sobreentrenamiento. Las imágenes de entrenamiento serán preprocesadas usando un filtro gaussiano y 'circle crop'.
...
guarda_graficas('Datasets','Si','Si','No','RGB','Mobeen',capas,n_filtros,n_neuronas,acc,loss,val_acc,val_loss)
...
with open('Resultados.csv','a') as fd:
    fd.write('\n')
    fd.write(f'Datasets,Sí,Sí,No,RGB,Mobeen,{capas},{n_filtros},{n_neuronas},iphone,{metricas_iphone[1]},{metricas_iphone[2]},{metricas_iphone[3]},{metricas_iphone[4]},{metricas_iphone[5]},{(fin-inicio)/60}')
...
with open('Resultados.csv','a') as fd:
    fd.write('\n')
    fd.write(f'Datasets,Sí,Sí,No,RGB,Mobeen,{capas},{n_filtros},{n_neuronas},Samsung,{metricas_samsung[1]},{metricas_samsung[2]},{metricas_samsung[3]},{metricas_samsung[4]},{metricas_samsung[5]},{(fin-inicio)/60}')
...
torch.save(modelo.state_dict(), f'modelos/Mobeen/Datasets_Sival_Siprep_Noinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')
