...
Para entrenar los modelos se usarán imágenes preprocesadas de OCT + Samsung o iPhone y se testearán con el conjunto de imágenes no empleado en el entrenamiento (iPhone o Samsung respectivamente).
...
root_dirs_OCT_S = ['Datos/Classified Data/Images_Proc/Samsung/No_inpaint', 'Datos/Classified Data/Images_Proc/OCT']
...
root_dirs_OCT_i = ['Datos/Classified Data/Images_Proc/iPhone/No_inpaint', 'Datos/Classified Data/Images_Proc/OCT']
...
for capas in [3,5,7]:
    ...
    for n_filtros in [1.0,0.5,2.0]:
        ...
        for n_neuronas in ['512/256','1024/512','256/128','128/64','64/128','128/256']:
            ...
            print('--------------------------------------')
            print(f'Entrenamiento. Características OCT+Samsung:\n  -Capas:{capas}\n  -Filtros:{n_filtros}\n  -Neuronas:{n_neuronas}\n  -Early Stopping\n')
            ...
            guarda_graficas('OCT_S','Si','Si','No','RGB','Rajagopalan',capas,n_filtros,n_neuronas,acc,loss,val_acc,val_loss)
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT_S,Sí,Sí,No,RGB,Rajagopalan,{capas},{n_filtros},{n_neuronas},iphone,{metricas_iphone[1]},{metricas_iphone[2]},{metricas_iphone[3]},{metricas_iphone[4]},{metricas_iphone[5]},{(fin-inicio)/60}')
            ...
            torch.save(modelo.state_dict(), f'modelos/Rajagopalan/OCT_S_Sival_Siprep_Noinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')
            ...
            #previo al entrenamiento imprimimos por pantalla las características de la red, para poder identificar su entrenamiento
            print('--------------------------------------')
            print(f'Entrenamiento. Características OCT+iPhone:\n  -Capas:{capas}\n  -Filtros:{n_filtros}\n  -Neuronas:{n_neuronas}\n  -Early Stopping\n')
            #capturamos el tiempo previo al entrenamiento
            inicio = time.time()
            #entrenamos la red con 7 épocas de paciencia en Early Stopping y guardamos los valores para poder representar las gráficas
            ...
            #guardamos las gráficas
            guarda_graficas('OCT_i','Si','Si','No','RGB','Rajagopalan',capas,n_filtros,n_neuronas,acc,loss,val_acc,val_loss)
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT_i,Sí,Sí,No,RGB,Rajagopalan,{capas},{n_filtros},{n_neuronas},Samsung,{metricas_samsung[1]},{metricas_samsung[2]},{metricas_samsung[3]},{metricas_samsung[4]},{metricas_samsung[5]},{(fin-inicio)/60}')
                
            #por último vamos a guardar el modelo, sus pesos y estado actual, por si se quisiera volver a emplear
            #primero para ello debemos cambiar el String de filtros y neuronas para evitar los puntos y barras laterales
            filtros_str = str(n_filtros).replace(".","punto")
            neuronas_str = str(n_neuronas).replace("/","slash")
            torch.save(modelo.state_dict(), f'modelos/Rajagopalan/OCT_i_Sival_Siprep_Noinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')
