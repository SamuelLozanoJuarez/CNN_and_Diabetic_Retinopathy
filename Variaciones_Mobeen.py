...
A continuación se incluye el código que permite crear varios modelos según la arquitectura propuesta en el artículo de Mobeen, pero realizando las modificaciones
...
#importamos el paquete para el cálculo del tiempo de ejecución
import time
    ...
    for n_filtros in [1.0,0.5,2.0]:
        ...
        for n_neuronas in ['100/50','256/128','128/64','64/32','32/64','64/128']:
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT,No,No,No,RGB,Mobeen,{capas},{n_filtros},{n_neuronas},iphone,{metricas_iphone[1]},{metricas_iphone[2]},{metricas_iphone[3]},{metricas_iphone[4]},{metricas_iphone[5]},{(fin-inicio)/60}')
            ...
            #para ello es necesario redefinir el valor de filtros y neuronas, para eliminar los puntos y barras laterales que no serían permitidos en el nombre del archivo
            filtros_str = str(filtros).replace('.','punto')
            neuronas_str = str(neuronas).replace('/','slash')
            torch.save(modelo.state_dict(), f'modelos/Mobeen/OCT_Noval_Noprep_Noinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')
