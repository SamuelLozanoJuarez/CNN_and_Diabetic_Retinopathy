...
Para el entrenamiento se usarán imágenes preprocesadas y un conjunto de datos de validación y se empleará la estrategia de Early Stopping, para evitar el sobreentrenamiento.
...
Para realizar el test de los modelos se usarán imágenes de Samsung e iPhone inpaintadas, procesadas para eliminar el flash de la imagen.
...
OCT = ImageFolder(root = 'Datos/Classified Data/Images_Proc/OCT', transform = transform)
...
for capas in [2,4,6]:
    ...
    for n_filtros in [1.0,0.5,2.0]:
        ...
        for n_neuronas in ['0/0','512/256','128/64','64/128','128/256']:
            ...
            guarda_graficas('OCT','Si','Si','Si','RGB','Alqudah',capas,n_filtros,n_neuronas,acc,loss,val_acc,val_loss)
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT,Sí,Sí,Sí,RGB,Alqudah,{capas},{n_filtros},{n_neuronas},iphone,{metricas_iphone[1]},{metricas_iphone[2]},{metricas_iphone[3]},{metricas_iphone[4]},{metricas_iphone[5]},{(fin-inicio)/60}')
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT,Sí,Sí,Sí,RGB,Alqudah,{capas},{n_filtros},{n_neuronas},Samsung,{metricas_samsung[1]},{metricas_samsung[2]},{metricas_samsung[3]},{metricas_samsung[4]},{metricas_samsung[5]},{(fin-inicio)/60}')
            ...
            torch.save(modelo.state_dict(), f'modelos/Alqudah/OCT_Sival_Siprep_Siinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')