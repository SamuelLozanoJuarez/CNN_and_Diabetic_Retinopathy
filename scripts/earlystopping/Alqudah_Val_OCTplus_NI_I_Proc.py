...
for capas in [2,4,6]:
    ...
    for n_filtros in [1.0,0.5,2.0]:
        ...
        for n_neuronas in ['0/0','512/256','128/64','64/128','128/256']:
            ...
            guarda_graficas('OCT_S','Si','Si','Si','RGB','Alqudah',capas,n_filtros,n_neuronas,acc,loss,val_acc,val_loss)
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT_S,Sí,Sí,Sí,RGB,Alqudah,{capas},{n_filtros},{n_neuronas},iphone,{metricas_iphone[1]},{metricas_iphone[2]},{metricas_iphone[3]},{metricas_iphone[4]},{metricas_iphone[5]},{(fin-inicio)/60}')
            ...
            torch.save(modelo.state_dict(), f'modelos/Alqudah/OCT_S_Sival_Siprep_Siinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')
            ...
            #guardamos las gráficas
            guarda_graficas('OCT_i','Si','Si','Si','RGB','Alqudah',capas,n_filtros,n_neuronas,acc,loss,val_acc,val_loss)
            ...
            with open('Resultados.csv','a') as fd:
                fd.write('\n')
                fd.write(f'OCT_i,Sí,Sí,Sí,RGB,Alqudah,{capas},{n_filtros},{n_neuronas},Samsung,{metricas_samsung[1]},{metricas_samsung[2]},{metricas_samsung[3]},{metricas_samsung[4]},{metricas_samsung[5]},{(fin-inicio)/60}')
                
            #por último vamos a guardar el modelo, sus pesos y estado actual, por si se quisiera volver a emplear
            #primero para ello debemos cambiar el String de filtros y neuronas para evitar los puntos y barras laterales
            filtros_str = str(n_filtros).replace(".","punto")
            neuronas_str = str(n_neuronas).replace("/","slash")
            torch.save(modelo.state_dict(), f'modelos/Alqudah/OCT_i_Sival_Siprep_Siinp_RGB_{capas}_{filtros_str}_{neuronas_str}.pth')