...
Para entrenar los modelos se usarán imágenes procesadas de OCT + Samsung o iPhone y se testearán con el conjunto de imágenes no empleado en el entrenamiento (iPhone o Samsung respectivamente).
...
guarda_graficas('OCT_S','Si','Si','No','RGB','Basica',2,1.0,'120/84',acc,loss,val_acc,val_loss)
...
with open('Resultados.csv','a') as fd:
    fd.write('\n')
    fd.write(f'OCTplus,Sí,Sí,No,RGB,Básica,2,1.0,120/84,iphone,{metricas_iphone[1]},{metricas_iphone[2]},{metricas_iphone[3]},{metricas_iphone[4]},{metricas_iphone[5]},{(fin-inicio)/60}')

#por último vamos a guardar el modelo, sus pesos y estado actual, por si se quisiera volver a emplear
torch.save(cnn.state_dict(), f'modelos/Basica/OCT_S_Sival_Siprep_Noinp_RGB.pth')
...
fin = time.time()
#posteriormente generamos y almacenamos las gráficas correspondientes a estas métricas
guarda_graficas('OCT_i','Si','Si','No','RGB','Basica',2,1.0,'120/84',acc,loss,val_acc,val_loss)
...
with open('Resultados.csv','a') as fd:
    fd.write('\n')
    fd.write(f'OCT_i,Sí,Sí,No,RGB,Básica,2,1.0,120/84,Samsung,{metricas_samsung[1]},{metricas_samsung[2]},{metricas_samsung[3]},{metricas_samsung[4]},{metricas_samsung[5]},{(fin-inicio)/60}')

#por último vamos a guardar el modelo, sus pesos y estado actual, por si se quisiera volver a emplear
torch.save(cnn.state_dict(), f'modelos/Basica/OCT_i_Sival_Siprep_Noinp_RGB.pth')